...
shinyServer(function(input,output,session)({
  ...
  output$entire_dataset <- DT::renderDataTable({
    data_with_titles<-datatable(data_with_titles)
    data_with_titles
  })
  
  output$downloadData_entire <- downloadHandler(
    filename = function() {
      paste("entire_dataset", ".csv", sep = "")
    },
    content = function(file) {
      write.csv(data_with_titles, file, row.names = FALSE)
    }
  )
  
  # output$filtered_dataset_genre <- DT::renderDataTable({
  #   data_with_titles<-datatable(data_with_titles)
  #   genre_filtered_data<-data_with_titles %>% filter(first_Genre == input$select_filter_genre)
  #   genre_filtered_data
  # })
  
  df_subset <- reactive({
    a <- subset(data_with_titles, first_Genre == input$select_filter_genre)
    return(a)
  })
  
  output$filtered_dataset_genre <- renderTable(df_subset()) #Note how df_subset() was used and not df_subset
  
  
  output$downloadData <- downloadHandler(
    filename = function() {
      paste(input$select_filter_genre, ".csv", sep = "")
    },
    content = function(file) {
  ...
  df_subset_rating <- reactive({
    b <- subset(data_with_titles, Rated == input$select_filter_rating)
    return(b)
  })
  
  output$filtered_dataset_rating <- renderTable(df_subset_rating()) #Note how df_subset() was used and not df_subset
  
  output$downloadData_rating <- downloadHandler(
    filename = function() {
      paste(input$select_filter_rating, ".csv", sep = "")
    },
    content = function(file) {
      write.csv(df_subset_rating(), file, row.names = FALSE)
    }
  )
  ...
  f <- reactive({
    if(input$random_forest_predictor!="Use All Variables"){
      as.formula(paste(input$random_forest_response, "~",paste(input$random_forest_predictor, "+", input$random_forest_predictor_2)))
  ...
  observeEvent(input$execute, {
    ...
    output$rf_test_rmse <- renderText({
      rf_test <- predict(rf$random.forest, newdata = dplyr::select(vals$testing, -input$random_forest_response))
      paste0("For the Random Forest, the RMSE on the testing data is ",
             RMSE(
               pred = rf_test,
  ...
  g <- reactive({
    if(input$tree_predictor!="Use All Variables"){
      as.formula(paste(input$tree_response, "~",paste(input$tree_predictor, "+", input$tree_predictor_2)))
  ...
  observeEvent(input$execute, {
    ...
    output$rt_test_rmse <- renderText({
      rt_test <- predict(rt$dtree_fit, newdata = dplyr::select(nums$rt_testing, -input$tree_response))
      paste0("For the regression tree, the RMSE on the testing data is ",
             RMSE(
               pred = rt_test,
               obs = nums$rt_testing[[input$tree_response]]
             )
      )
    })
  ...
  h <- reactive({
    if(input$linear_predictor!="Use All Variables"){
      as.formula(paste(input$linear_response, "~",paste(input$linear_predictor, "+", input$linear_predictor_2)))
  ...
  observeEvent(input$execute, {
    ...
    output$lm_test_rmse <- renderText({
      lm_test <- predict(linear_model$lm.model, newdata = dplyr::select(quant$lm_testing, -input$linear_response))
      paste0("For the Linear Model, the RMSE on the testing data is ",
             RMSE(
               pred = lm_test,
               obs = quant$lm_testing[[input$linear_response]]
             )
      )
    })
...
#   h <- reactive({
#     if(input$linear_predictor!="Use All Variables"){
#       #as.formula(paste(input$linear_response, "~",paste(input$linear_predictor)))
#       as.formula(paste(input$linear_response, "~",paste(input$linear_predictor,collapse="+")))
#     } else if(input$linear_predictor=="Use All Variables"){
#       as.formula(paste(input$linear_response, "~."))
#     }
